---
name: popiskill-image-text2image-qwen-v1
description: Generate images through Qwen Image. Use this when the user wants text-to-image generation with strong Chinese prompt support and a simple PopiArt run path.
...
With custom model:
Support `popiskill-image-text2image-qwen-v1-max-2025-12-30` `popiskill-image-text2image-qwen-v1-plus-2026-01-09` `popiskill-image-text2image-qwen-v1-plus`
```bash
uv run {baseDir}/scripts/generate_image.py --prompt "a beautiful sunset over mountains" --model popiskill-image-text2image-qwen-v1-plus-2026-01-09 --api-key sk-xxx
```
...
- Or get from `skills."popiskill-image-text2image-qwen-v1".apiKey` in `~/.openclaw/openclaw.json`
