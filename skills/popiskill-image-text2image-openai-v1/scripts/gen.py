...
def _default_out_dir() -> str:
    ...
    if os.path.isdir(projects_tmp):
        return os.path.join(projects_tmp, f"popiskill-image-text2image-openai-v1-{_stamp()}")
    return os.path.join(os.getcwd(), "tmp", f"popiskill-image-text2image-openai-v1-{_stamp()}")
...
def _write_index(out_dir: str, items: list[dict]) -> None:
    html = [
        ...
        "<title>popiskill-image-text2image-openai-v1</title>",
        "<style>",
        ...
        "</style>",
        "<h1>popiskill-image-text2image-openai-v1</h1>",
    ]
    for it in items:
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        prog="popiskill-image-text2image-openai-v1",
